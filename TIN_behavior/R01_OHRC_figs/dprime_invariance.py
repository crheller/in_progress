...
tar_mask = (df.tar_tar) & (df.tdr_overall==False) & (df.f1 == df.f2) & (~df.pca)
cat_mask = (df.cat_tar) & (df.tdr_overall==False) & (df.f1 == df.f2) & (~df.pca)
...
ax[0, 1].scatter(df[cat_mask & df.active & (df.area=='PEG')].groupby(by='site').mean()[val],
            df[cat_mask & ~df.active & (df.area=='PEG')].groupby(by='site').mean()[val], s=80, edgecolor='k', label='Tar vs. Cat')
...
for i, s in enumerate(list(set(ct_delta.index).intersection(set(tt_delta.index)))):
    ...
    ax[1, 0].errorbar([0, 1], [tt_delta.loc[s].values[0], ct_delta.loc[s].values[0]], \
        yerr=[tt_sem.loc[s].values[0], ct_sem.loc[s].values[0]], capsize=3, color=colors(i), label=lab, marker='o')

leg = ax[1, 0].legend(frameon=False, handlelength=0)
for line, text in zip(leg.get_lines(), leg.get_texts()):
    text.set_color(line.get_color())
ax[1, 0].set_xticks([0, 1])
ax[1, 0].set_xticklabels(['Tar. vs. Tar', 'Cat vs. Tar'], rotation=45)
...
for i, s in enumerate(list(set(ct_delta.index).intersection(set(tt_delta.index)))):
    ncells = len([c for c in cells if s in c])
    lab = s + f' ({ncells} cells)'
    ax[1, 1].errorbar([0, 1], [tt_delta.loc[s].values[0], ct_delta.loc[s].values[0]], \
        yerr=[tt_sem.loc[s].values[0], ct_sem.loc[s].values[0]], capsize=3, color=colors(i), label=lab, marker='o')
...
ax[1, 1].set_xticklabels(['Tar. vs. Tar', 'Cat vs. Tar'], rotation=45)
